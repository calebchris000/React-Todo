...
const Todo = (props) => {
    ...
    useEffect(() => {
        ...
        todo.forEach((item, index) => {
            obj.push({ item: item, index: index, isCompleted: local[index].isCompleted });
        })
        if (obj.length !== 0) {
    ...
    return (
        <div className='todoWrapper'>
            ...
            <div className='itemWrap'>
                {
                    todo.map((i, index) => <div className='item' key={index}>
                        <div className='checkWrapper'>
                            <input className='check' onChange={() => handleCheckBox(index)} type={'checkbox'} defaultChecked={inputChecked(index)} />
                            <p>{i}</p>
                        </div>

                        <button onClick={() => handleDelete(index)} className='trash'><FaTrash /></button>
                    </div>)}
            </div>
        </div>
    )
